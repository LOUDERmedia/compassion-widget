...
class Loud_Compassion_Banner_Widget extends WP_Widget {	
	...
	function Loud_Compassion_Banner_Widget() {
		...
		$control_ops = array( 'width' => 350, 'id_base' => 'loud-compassion-banner-widget' );
	...
	function admin_head() {
		...
		<script type="text/javascript">
			jQuery(document).ready(function($) {
				if ($('#<?php echo $this->get_field_id( 'use_source_code' ); ?>').attr('checked') == false) {
					$('#<?php echo $this->get_field_id( 'source_code' ); ?>_wrap').hide();
				}
				$('#<?php echo $this->get_field_id( 'use_source_code' ); ?>').live('click', function() {
					var scvalue = $(this).attr('checked');
					$('#<?php echo $this->get_field_id( 'source_code' ); ?>_wrap').toggle(scvalue);
				});
				$('.select_banner_button').live('click', function () {
						...
						$('#<?php echo $this->get_field_id('banner_key'); ?> option').attr('selected','');
						$('#<?php echo $this->get_field_id('banner_key'); ?> option[value="'+choice+'"]').attr('selected','selected');
					}
				);
				$('#<?php echo $this->get_field_id('show_children_from'); ?>').live('change', function () {
					var country = $(this).val();
					var newLinkHtml = '<a class="thickbox button" href="<?php echo get_option('siteurl'); ?>/wp-admin/admin-ajax.php?action=bannerSelect&countryfield='+country+'&width=800&height=700" title="Select a Banner" id="show_banners_button_all">View Available Banners</a>';
					if (!country.length) {
						var newBannerList = $('div#loud_cw_banners').html();
					} else {
						var newBannerList = $('div#loud_cw_country_banners').html();
					}
					$('select#<?php echo $this->get_field_id('banner_key'); ?>').html(newBannerList);
					$('p#banners_link_wrap').html(newLinkHtml);
				})
				.change();
			});
		</script>
	...
	function bannerSelect() {
		...
		$country_field = $_GET['countryfield'];
		...
		foreach ($current_banners as $bannerkey => $banner){
			...
			$banner_output.='<div class="banner_preview"><iframe src="'.WP_PLUGIN_URL.'/compassion-widget/loud-inc/frame.php?bannerkey='.$bannerkey.'" width="100%" height="'.($banner['height']+20).'"></iframe></div>';
	...
	function widget( $args, $instance ) {
		...
		if (!empty($add_text_link)) {
			$output .= '<p style="text-align:center;"><a href="'.$loud_cw_text_links[$add_text_link].$referer_code.'" title="'.$add_text_link.'">'.$add_text_link.'</a></p>'."\n";	
		}
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['banner_key'] = $new_instance['banner_key'];
	...
	function form( $instance ) {
		...
		$defaults = array('title' => 'Sponsor a Child', 'use_source_code' => 'off', 'source_code' => '', 'show_children_from' => '', 'banner_key' => $loud_cw_banners_keys[0], 'add_text_link' => '' );
		...
		<p>
			<input type="checkbox" id="<?php echo $this->get_field_id( 'use_source_code' ); ?>" name="<?php echo $this->get_field_name( 'use_source_code' ); ?>" value="on" class="checkbox"<?php if ($instance['use_source_code'] == 'on') echo ' checked="checked" '; ?>/>
			<label for="<?php echo $this->get_field_id( 'use_source_code' ); ?>">Use Source Code?</label><br />
			<div id="<?php echo $this->get_field_id( 'source_code' ); ?>_wrap"<?php if ($instance['use_source_code'] != 'on') echo ' style="display:none;"'; ?>><label for="<?php echo $this->get_field_id( 'source_code' ); ?>">Source Code:</label><input type="text" name="<?php echo $this->get_field_name( 'source_code' ) ?>" value="<?php echo $instance['source_code']; ?>" id="<?php echo $this->get_field_id( 'source_code' ); ?>"></div>
			
	 	</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'show_children_from' ) ?>">Show Children From:</label>
			<select name="<?php echo $this->get_field_name('show_children_from'); ?>" id="<?php echo $this->get_field_id('show_children_from'); ?>">
				<?php
		...
		<p id="banners_link_wrap">
			<a class="thickbox button" href="<?php echo get_option('siteurl'); ?>/wp-admin/admin-ajax.php?action=bannerSelect&countryfield=<?php echo $instance['show_children_from']; ?>&width=800&height=700" title="Select a Banner" id="show_banners_button_country">View Available Banners</a>
		</p>
		<p>
			<label for"<?php echo $this->get_field_id('banner_key'); ?>">Selected Banner:</label>
			<select name="<?php echo $this->get_field_name('banner_key'); ?>" id="<?php echo $this->get_field_id('banner_key'); ?>">
				<?php
				foreach ($current_banners as $bannerkey => $banner) {
					$output = '<option value="'.$bannerkey.'"';
					if ($bannerkey == $instance['banner_key']) $output.=' selected="selected"';
					$output.='>'.$banner['width'].'x'.$banner['height'].' - '.$banner['type'].'</option>';
					echo $output;	
				}
				?>				
			</select>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'add_text_link' ); ?>">Text Link:</label>
			<select name="<?php echo $this->get_field_name('add_text_link'); ?>" id="<?php echo $this->get_field_id('add_text_link'); ?>">
		...
		<div id="loud_cw_banners" style="display:none;">
			<?php
		...
		<div id="loud_cw_country_banners" style="display:none;">
			<?php
